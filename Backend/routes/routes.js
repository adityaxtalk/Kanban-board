...
api.get("/project/:id", async (req, res)=> {
    ...
    try {
        const data = await Project.find({_id :new  mongoose.mongo.ObjectId(req.params.id)}).sort({ order: 1});
        return res.send(data);
    } catch (error) {
        return res.send(error);
    }
...
api.put('/project/:id', async (req, res)=> {
    ...
    try {
        ...
        if (result.modifiedCount === 0 && !result.upserted) {
            return res.status(404).send({ error: true, message: 'No project found with that ID' });
        }
...
api.delete('/project/:id', async (req, res)=> {
    try {
        const data = await Project.deleteOne({_id : new  mongoose.mongo.ObjectId(req.params.id)});
        res.send(data);
    } catch (error) {
        res.send(error)
    }
...
const {error, value} = task.validate({title: req.body.title, description: req.body.description});
    ...
    try {
        const [{task}] = await Project.find({ _id: new  mongoose.mongo.ObjectId(req.params.id)}, {"task.index": 1}).sort({'task.index': 1})
        let countTaskLength = [task.length, task.length > 0 ? Math.max(...task.map(o=> o.index)): task.length];
        const data = await Project.updateOne({_id: new  mongoose.mongo.ObjectId(req.params.id)}, {$push: {task: {...value, stage: "Requested", order: countTaskLength[0], index: countTaskLength[1] + 1}}});
        return res.send(data);
    } catch(error) {
...
api.delete('/project/:id/task/:taskId', async (req, res)=> {
   ...
   try {
      const data= await Project.updateOne({ _id: new  mongoose.mongo.ObjectId(req.params.id)}, { $pull: {task: { _id: new mongoose.mongo.ObjectId(req.params.taskId)}}})
      return res.send(data);
   } catch (error) {
      return res.send(error)
   }
