...
const Task = () => {
    ...
    useEffect(()=> {
       ...
       if (!isAddTaskModalOpen || isRenderChange) {
            axios.get(`${process.env.REACT_APP_BASE_URI}/project/${projectId}`).then(res=> {
  ...
  const updateTodo = (data) => {
    axios.put(`${process.env.REACT_APP_BASE_URI}/project/${projectId}/todo`, data).then(res=> {}).catch(error=> {
        toast.error("Something went wrong");
    })
  }
  ...
  const handleDelete = (e,taskId) => {
    e.stopPropagation();
    axios.delete(`${process.env.REACT_APP_BASE_URI}/project/${projectId}/task/${taskId}`).then(res=> {
    ...
    <div className='px-12 py-6 w-full'>
        <div className='flex items-center justify-between mb-6'>
            <h1 className='text-xl text-gray-800 flex justify-start items-center space-x-2.5'>
                ...
                <ProjectDropdown id={projectId} navigate={navigate} setRenderChange={setRenderChange}/>
            </h1>
