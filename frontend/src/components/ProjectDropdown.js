...
const ProjectDropdown = ({id, navigate, setRenderChange}) => {

  const [isModalOpen, setModalState] = useState(false);
  const handleDelete = async () => {
    try {
        const data = await axios.delete(`${process.env.REACT_APP_BASE_URI}/project/${id}`);
  ...
  return (
    <>
      <Popover className="relative">
        <PopoverButton className='duration-75 rounded-sm focus:outline-none'>
